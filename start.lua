...
settings.line.centerxr              = settings.line.startx -
    settings.line
    .width_2 -- right of center; left of the right column (2 columns)
settings.line.centerxl              = settings.line.centerxr -
    settings.base
    .margin                                                -- left of center; right of the left column (2 columns)
settings.line.endx                  = settings.base.margin -- left
settings.line.rightxl               = settings.line.startx -
    settings.line
    .width_3 -- left of the right column (3 columns)
settings.line.midxr                 = settings.line.rightxl -
    settings.base
    .margin -- right of the mid column (3 columns)
settings.line.midxl                 = settings.line.midxr -
    settings.line
    .width_3 -- left of the mid column (3 columns)
settings.line.leftxr                = settings.line.midxl -
    settings.base
    .margin -- right of the left column (3 columns)
settings.line.utilxl                = settings.line.startx -
    settings.line
    .width_4 -- left of the right column (4 columns)
settings.line.utilxr                = settings.line.utilxl -
    settings.base
    .margin -- right of the mid column (4 columns)
...
settings.text.centerxr              = settings.line.centerxr +
    settings.base.margin /
    2 -- right of center; left of the right column (2 columns)
settings.text.centerxl              = settings.line.centerxl -
    settings.base.margin /
    2                                                                               -- left of center; right of the left column (2 columns)
settings.text.endx                  = settings.line.endx + settings.base.margin / 2 -- left
settings.text.rightxl               = settings.line.rightxl +
    settings.base.margin /
    2 -- left of the right column (3 columns)
settings.text.midxr                 = settings.line.midxr -
    settings.base.margin /
    2 -- right of the mid column (3 columns)
settings.text.midxl                 = settings.line.midxl +
    settings.base.margin /
    2 -- left of the mid column (3 columns)
settings.text.leftxr                = settings.line.leftxr -
    settings.base.margin /
    2 -- right of the left column (3 columns)
settings.text.utilxl                = settings.line.utilxl +
    settings.base.margin /
    2 -- left of the right column (4 columns)
settings.text.utilxr                = settings.line.utilxr -
    settings.base.margin /
    2 -- right of the mid column (4 columns)
...
function draw_clock()
    ...
    write(settings.clock.date_x, settings.clock.date_y,
        string.format("%s, %s %s %s", time_day(), time_month_number(), time_month(), time_year()), 20, main_text_color,
        "c")
end

function draw_info()
    write_bold(settings.line.endx, settings.info.separator + settings.line.height / 2, "SYSTEM", 12, main_text_color)
    line(settings.line.startx, settings.info.separator, settings.line.endx + 56, settings.info.separator,
...
function draw_temp()
    local cputemp = tonumber(cpu_temperature()) or 0
    local gputemp = tonumber(gpu_temp(0)) or 0
    local ssdtemp = tonumber(ssd_temperature()) or 0
    --local pchtemp  = tonumber(pch_temperature()) or 0
    --local acpitemp = tonumber(acpi_temperature()) or 0

    rectangle_bottomup(settings.temp.x1, settings.temp.y, settings.line.temp, settings.line.thickness, cputemp, 100,
        color_frompercent(cputemp / 100))
    ...
    rectangle_bottomup(settings.temp.x2, settings.temp.y, settings.line.temp, settings.line.thickness, gputemp, 95,
        color_frompercent(gputemp / 95))
    ...
    rectangle_bottomup(settings.temp.x3, settings.temp.y, settings.line.temp, settings.line.thickness, ssdtemp, 86.8,
        color_frompercent(ssdtemp / 86.8))
...
function draw_cpu()
    write_bold(settings.line.endx, settings.cpu.separator + settings.line.height / 2, "CPU: " .. cpu_name(), 12,
        main_text_color)
    line(settings.line.startx, settings.cpu.separator, settings.line.endx + 308, settings.cpu.separator,
        settings.line.thickness, main_text_color, 1)
    ...
    for thread = 1, cpu_cores do
        ...
        rectangle_rightleft(settings.line.utilxr, y, settings.line.width_4, settings.line.thickness, cpu_percent(thread),
            100, color_frompercent(tonumber(cpu_percent(thread))))
    ...
    for thread = cpu_cores + 1, cpu_threads do
        ...
        rectangle_rightleft(settings.line.startx, y, settings.line.width_4, settings.line.thickness, cpu_percent(thread),
            100, color_frompercent(tonumber(cpu_percent(thread))))
...
function draw_gpu()
    ...
    local gpupowerdraw  = tonumber(gpu_power_draw(0)) or 0
    local gpupowerlimit = tonumber(gpu_power_limit(0)) or 75
    local gpuutil       = tonumber(gpu_utilization(0)) or 0
    local gpumemused    = tonumber(gpu_mem_used(0)) or 0
    local gpumemtotal   = tonumber(gpu_mem_total(0)) or 0
    local gpumemutil    = math.floor(gpumemused / gpumemtotal * 100)
    local gpuencode     = tonumber(gpu_encode(0)) or 0
    local gpudecode     = tonumber(gpu_decode(0)) or 0

    rectangle_rightleft(settings.line.centerxl, settings.gpu.y_fan, settings.line.width_2, settings.line.thickness,
        gpufan, 100, color_frompercent(gpufan))
    ...
    rectangle_rightleft(settings.line.centerxl, settings.gpu.y_power, settings.line.width_2, settings.line.thickness,
        gpupowerdraw, gpupowerlimit, color_frompercent(gpupowerdraw / gpupowerlimit))
    ...
    rectangle_rightleft(settings.line.centerxl, settings.gpu.y_ram, settings.line.width_2, settings.line.thickness,
        gpumemutil, 100, color_frompercent(gpumemutil))
    write(settings.text.endx, settings.gpu.y_ram - settings.line.height, gpumemutil .. " %", 12, main_text_color)
    write(settings.text.centerxl, settings.gpu.y_ram - settings.line.height,
        "RAM " .. gpumemused .. " / " .. gpumemtotal .. " MiB", 12, main_text_color, "r")

    rectangle_rightleft(settings.line.startx, settings.gpu.y_util, settings.line.width_2, settings.line.thickness,
        gpuutil, 100, color_frompercent(gpuutil))
    ...
    rectangle_rightleft(settings.line.startx, settings.gpu.y_decode, settings.line.width_2, settings.line.thickness,
        gpudecode, 100, color_frompercent(gpudecode))
    ...
    rectangle_rightleft(settings.line.startx, settings.gpu.y_encode, settings.line.width_2, settings.line.thickness,
        gpuencode, 100, color_frompercent(gpuencode))
...
function draw_memory()
    ...
    line(settings.line.startx, settings.mem.separator, settings.line.endx + 52, settings.mem.separator,
        settings.line.thickness, main_text_color, 1)
    ...
    rectangle_rightleft(settings.line.startx, settings.mem.y_ram, settings.line.width_2, settings.line.thickness, memperc,
        100, color_frompercent(tonumber(memperc)))
    ...
    rectangle_rightleft(settings.line.startx, settings.mem.y_swap, settings.line.width_2, settings.line.thickness,
        swapperc, 100, color_frompercent(tonumber(swapperc)))
    ...
    write_line_by_line(settings.text.centerxr, settings.mem.y_usage, settings.line.info_height, vals, main_text_color, 12,
        false)
end

function draw_net()
    write_bold(settings.line.endx, settings.net.separator + settings.line.height / 2, "NETWORK", 12, main_text_color)
    line(settings.line.startx, settings.net.separator, settings.line.endx + 60, settings.net.separator,
        settings.line.thickness, main_text_color, 1)
...
function draw_data()
    ...
    line(settings.line.startx, settings.data.separator, settings.line.endx + 60, settings.data.separator,
        settings.line.thickness, main_text_color, 1)
    ...
    rectangle_rightleft(settings.line.startx, settings.data.y_ssd_util, settings.line.width_5, settings.line.thickness,
        rootperc, 100, color_frompercent(rootperc))
    ...
    -- Network volumes
    local jodibooks     = string.format("/media/jodibooks-share: %s / %s", fs_used("/media/jodibooks-share"),
        fs_size("/media/jodibooks-share"))
    local joediapna     = string.format("/media/joediapna-share: %s / %s", fs_used("/media/joediapna-share"),
        fs_size("/media/joediapna-share"))
    ...
    rectangle_rightleft(settings.line.startx, settings.data.y_sh1, settings.line.width_5, settings.line.thickness,
        jodibooksperc, 100, color_frompercent(jodibooksperc))
    ...
    rectangle_rightleft(settings.line.startx, settings.data.y_sh2, settings.line.width_5, settings.line.thickness,
        joediapnaperc, 100, color_frompercent(joediapnaperc))
    ...
    rectangle_rightleft(settings.line.startx, settings.data.y_sh3, settings.line.width_5, settings.line.thickness,
        userperc, 100, color_frompercent(userperc))
    ...
    write_line_by_line(settings.text.endx, settings.data.y_speeds, settings.line.info_height, vals, main_text_color, 12,
        false)
end
